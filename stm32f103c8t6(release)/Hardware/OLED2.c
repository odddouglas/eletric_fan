#include "stm32f10x.h"
#include "OLED2_Font.h"

/*引脚配置*/
#define OLED2_W_SCL(x)		GPIO_WriteBit(GPIOB, GPIO_Pin_3, (BitAction)(x))
#define OLED2_W_SDA(x)		GPIO_WriteBit(GPIOB, GPIO_Pin_4, (BitAction)(x))

/*引脚初始化*/
void OLED2_I2C_Init(void)
{
...
/**
  ...
  * @retval 无
  */
void OLED2_I2C_SendByte(uint8_t Byte)
{
	uint8_t i;
	for (i = 0; i < 8; i++)
	{
		OLED2_W_SDA(Byte & (0x80 >> i));
		OLED2_W_SCL(1);
		OLED2_W_SCL(0);
	}
	OLED2_W_SCL(1);	//额外的一个时钟，不处理应答信号
	OLED2_W_SCL(0);
}
...
/**
  ...
  * @retval 无
  */
void OLED2_WriteCommand(uint8_t Command)
{
	OLED2_I2C_Start();
	OLED2_I2C_SendByte(0x78);		//从机地址
	OLED2_I2C_SendByte(0x00);		//写命令
	OLED2_I2C_SendByte(Command); 
	OLED2_I2C_Stop();
}
...
/**
  ...
  * @retval 无
  */
void OLED2_WriteData(uint8_t Data)
{
	OLED2_I2C_Start();
	OLED2_I2C_SendByte(0x78);		//从机地址
	OLED2_I2C_SendByte(0x40);		//写数据
	OLED2_I2C_SendByte(Data);
	OLED2_I2C_Stop();
}
...
/**
  ...
  * @retval 无
  */
void OLED2_SetCursor(uint8_t Y, uint8_t X)
{
...
{      	
	...
	for (i = 0; i < 8; i++)
	{
		OLED2_WriteData(OLED2_F8x16[Char - ' '][i]);			//显示上半部分内容
	}
	OLED2_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
	for (i = 0; i < 8; i++)
	{
		OLED2_WriteData(OLED2_F8x16[Char - ' '][i + 8]);		//显示下半部分内容
	}
}
...
void OLED2_ShowNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED2_ShowChar(Line, Column + i, Number / OLED2_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
void OLED2_ShowSignedNum(uint8_t Line, uint8_t Column, int32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED2_ShowChar(Line, Column + i + 1, Number1 / OLED2_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
void OLED2_ShowHexNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)							
	{
		SingleNumber = Number / OLED2_Pow(16, Length - i - 1) % 16;
		if (SingleNumber < 10)
		{
			OLED2_ShowChar(Line, Column + i, SingleNumber + '0');
		}
		else
		{
			OLED2_ShowChar(Line, Column + i, SingleNumber - 10 + 'A');
		}
	}
}
...
void OLED2_ShowBinNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED2_ShowChar(Line, Column + i, Number / OLED2_Pow(2, Length - i - 1) % 2 + '0');
	}
}
